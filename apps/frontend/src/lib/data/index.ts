...
import { FRANCHISE_ORDER } from "./franchises";
...
export async function getProducts(
  ...
  const slugs = franchise ? (Array.isArray(franchise) ? franchise : [franchise]) : FRANCHISE_ORDER;

  const rows = await fetchAllProducts();
  const all = rows
    .filter((p) => slugs.includes(p.franchise_slug as FranchiseSlug))
...
export async function getFeaturedProducts(limit = 8, locale: Locale = "en"): Promise<Product[]> {
  ...
  for (const item of items) {
    ...
    if (seen.has(item.franchise) && seen.size < FRANCHISE_ORDER.length) continue;
...
export async function getSettings(): Promise<SiteSettings> {
  ...
  const { data, error } = await supabase
    ...
    .eq("id", true)
    .maybeSingle();
  if (error || !data) return { currency: "USD", whatsappNumber: null, contactEmail: null };
  return {
